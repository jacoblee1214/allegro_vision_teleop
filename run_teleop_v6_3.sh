#!/usr/bin/env bash
# ==============================================================================
# run_teleop_v6_3.sh — All-in-One Launcher for Allegro Hand V6 Vision Teleoperation (v6_3)
#
# v6_3 vs v6_2: teleop_cockpit_v6_3.py restores the GL state every frame (QPainter HUD disabled depth test
#   after the first frame -> inside-out rendering, left hand looked like a right hand) and uses the RViz default view.
#
# v6_2 vs v6_1: cockpit 3D view renders on the NVIDIA GPU via PRIME render offload
#   (software llvmpipe: ~50 ms/frame for the 476k-triangle STL hand -> ~1.4 ms on GPU).
#   Falls back to software GL when no NVIDIA GPU is visible in the container.
#   teleop_cockpit_v6_2.py: left-hand 3D view seen from the palm side (mirror of right). Other Python nodes are v6_1.
#
...
if [ ! -f "/.dockerenv" ] && [ "${RUN_ON_HOST:-0}" != "1" ]; then
    ...
    exec docker exec $DOCKER_FLAGS \
        -e DISPLAY="${DISPLAY:-:1}" \
        ros_humble_dev /home/humble_ws/allegro_vision_teleop/run_teleop_v6_3.sh "$@"
fi
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-rviz)
            ...
            ;;
        -h|--help)
            echo "Usage: ./run_v6_3.sh [nodes|sim|real] [UI options] [HW options]"
            echo ""
            echo "Modes:"
        ...
        *)
            ...
            echo "Run './run_v6_3.sh --help' for usage."
            exit 1
            ;;
    esac
done
...
# Cockpit only: render the embedded 3D hand on the NVIDIA GPU (display is driven by the Intel iGPU,
# whose Mesa driver is unavailable in the container, hence the software default above)
COCKPIT_GL_ENV=""
if nvidia-smi >/dev/null 2>&1; then
    COCKPIT_GL_ENV="env -u LIBGL_ALWAYS_SOFTWARE __NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia"
fi
...
pkill -f 'teleop_cockpit_v6(_[123])?\.py|teleop_dashboard_v6(_1)?\.py|vision_tracker_v6\.py' 2>/dev/null || true
sleep 0.2
...
" 2>/dev/null || true)
        ...
        if [[ "$DESCRIPTOR" =~ modbus_tcp:([0-9.]+):([0-9]+) ]]; then
            ...
            if ! ping -c 1 -W 1 "$TARGET_IP" >/dev/null 2>&1; then
                ...
                echo "[!] (Tip: If testing without physical hand, run 'run_v6_3 sim' instead)"
                echo "--------------------------------------------------------"
            else
...
case "$UI_MODE" in
    cockpit)
        ...
        $COCKPIT_GL_ENV python3 "$SCRIPT_DIR/teleop_cockpit_v6_3.py" --device "$DEVICE" --fps "$FPS" --hand "$HAND_SIDE" --mode "$MODE" &
        PIDS+=($!)
        ;;
    classic)
