...
echo "[+] Starting Kinematic Retargeting Node..."
python3 "$SCRIPT_DIR/retargeting_node.py" --quiet &
PIDS+=($!)
sleep 0.5

echo "[+] Starting Controller Bridge Node (EMA alpha=$ALPHA)..."
python3 "$SCRIPT_DIR/sim_bridge_node.py" --alpha "$ALPHA" &
PIDS+=($!)
sleep 0.5

echo "[+] Starting Vision Tracker Node (/dev/video$DEVICE)..."
python3 "$SCRIPT_DIR/vision_tracker.py" --device "$DEVICE" $GUI_FLAG &
PIDS+=($!)
