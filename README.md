...
### 3. 🛡️ Vision2Real 기구학 1:1 표준 일치 원리 (CAD 순정 URDF & 무왜곡 직결 제어)
> **"왜 인위적인 모터 오프셋(-90도 등)을 일절 두지 않고, 비전과 하드웨어를 1:1 직결해야 하는가?"**
>
> 향후 xArm7 등 매니퓰레이터 로봇 팔과 핸드를 결합하여 상체 전체(Arm+Hand) 역기구학(IK/FK) 및 모션 플래닝을 수행할 때, 손 모델의 원점이나 제어 입력값에 인위적인 각도 왜곡이 들어가 있으면 전체 기구학 체인이 완전히 파탄납니다.
> 실측 엔코더 분석 결과, 원익 Allegro Hand V6의 기계적 0.0 rad 위치는 손가락이 위로 곧게 펴진(Straight Open) 표준 상태와 완벽히 일치합니다. 따라서 본 시스템은 **Vision2Real 1:1 직결 원칙**을 엄격히 고수합니다:

```mermaid
flowchart LR
    A["🎥 비전 트래커<br/>(MediaPipe 21 3D)"] -->|관절 각도 계산| B["📐 순정 URDF 표준 좌표계<br/>• 손 폈을 때: 0.00 rad (완전 직립 신전)<br/>• 주먹 쥘 때: +1.57 rad (손바닥 방향 굽힘)"]
    B -->|동일한 표준 각도 전달| C["🖥️ UI & 3D 뷰어 (RViz / Cockpit CAD)<br/>• 손가락 똑바로 직립 신전<br/>• 뒤로 꺾임 0% 완전 박멸"]
    B -->|1:1 무왜곡 직결 송출| D["⚡ sim_bridge_node (100Hz EMA)<br/>• 인위적 오프셋 제거 (순정 1:1 패스스루)"]
    D -->|실물 모터 수신| E["🦾 Allegro Hand V6 실물 로봇<br/>• 0.00 rad 수신 시 손가락 곧게 쫙 펴짐<br/>• 비전과 물리 로봇 완벽 1:1 일치!"]
```

1. **URDF 및 비전 리타게팅의 순정 표준화**:
   - `urdf/allegro_hand_v6_left.urdf`는 원익 공식 CAD 원본 그대로 `lower="-0.0698132" upper="1.5707963"`를 유지합니다.
   - `retargeting_node_v6.py`는 사용자가 손을 폈을 때 `0.00 rad` (완전 직립 신전), 접을 때 `0.0 ~ +1.57 rad`를 정확히 산출합니다.
   - **RViz2 및 3D Cockpit 위젯에서 손가락이 뒤로 꺾이지 않고 하늘을 향해 곧게 펴진 완벽한 영점(Zero Position) 자세**를 유지합니다.
2. **실물 로봇 및 시뮬레이션의 완벽한 1:1 정합 (`sim_bridge_node_v6.py`)**:
   - 기존의 잘못된 인위적 모터 오프셋(-90도)을 완전히 제거하여, 비전에서 인식한 손 자세가 RViz, 3D Cockpit, 물리 로봇 손에 100% 동일한 각도로 전달됩니다.
   - 결과: **켜자마자 손이 쫙 펴진 상태(Zero Position)로 시작되며, 뒤로 꺾임 없이 전면 굽힘 및 핀치 조작이 매끄럽게 수행**됩니다.
