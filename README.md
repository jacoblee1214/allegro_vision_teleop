...
|---|---|---|---|
| **`joint00`** | Base Opposition (엄지 1번째: 대립/외전) | 대립 시 **양수(`+0.05 ~ +1.40 rad`)** | 대립 시 **양수(`+0.05 ~ +1.40 rad`)**<br/>*(검지 방향 손바닥 전면 스윕, 뒤로 꺾임 0% 원천 방지)* |
...
### 3. 🛡️ Vision2Real vs Simulation URDF 정합 원리 (시뮬레이션과 물리 로봇의 완전 분리)
> **"왜 시뮬레이션(URDF)과 실물 로봇(Real Hardware)의 MCP 관절 오프셋을 분리 제어해야 하는가?"**
>
> 1. **시뮬레이션(URDF / RViz / 3D CAD 위젯)**: URDF 모델은 `0.0 rad`가 손가락이 하늘을 향해 똑바로 펴진(Straight Open) 표준 기구학 상태입니다. 시뮬레이션 모드(`sim`)에서는 인위적인 오프셋 없이 비전 인식 각도(`0.0 rad`)를 그대로 수신해야 가상 환경에서 손가락이 뒤로 꺾이지(Hyperextension) 않습니다.
> 2. **실물 로봇(Physical Hardware, 왼손)**: 원익 로보틱스 하드웨어 개발자가 왼손 MCP 모터(`joint11, 21, 31, 41`)의 기계적 영점 기준을 손가락이 쫙 펴진 상태에서 `-90°` (`-1.5708 rad`)가 되도록 의도적으로 캘리브레이션해 두었습니다. 따라서 실물 제어 모드(`real`)에서는 모터 송출 시에만 이 `-90°` 오프셋을 정확히 인가해야 실물 손가락이 똑바로 펴집니다.

```mermaid
flowchart TD
    A["🎥 비전 트래커<br/>(MediaPipe 21 3D)"] -->|키네마틱스 연산| B["📐 순정 URDF 표준 좌표계<br/>• 손 폈을 때: 0.00 rad (직립 신전)<br/>• 주먹 쥘 때: +1.57 rad (손바닥 방향 굽힘)"]
    
    B -->|sim 모드 (1:1 표준 송출)| C["🖥️ 가상 시뮬레이션 & RViz2<br/>• 0.00 rad 수신<br/>• 손가락 직립 유지 (뒤로 꺾임 0%)"]
    
    B -->|real 모드 (왼손 MCP -90° 적용)| D["⚡ sim_bridge_node (100Hz EMA)<br/>• cmd_data = target - 1.5708 rad"]
    D -->|실물 모터 송출| E["🦾 Allegro Hand V6 실물 로봇<br/>• 모터 -90° 도달 시 손가락 곧게 쫙 펴짐!"]
    
    E -->|실물 엔코더 피드백 (-1.57 rad)| F["🔄 Cockpit / Dashboard 3D 위젯<br/>• val += 1.57 rad 역변환 정규화<br/>• 3D 화면에서도 완벽한 직립 0.0 rad 렌더링"]
```

1. **기동 즉시 영점 자세(Zero Position, 쫙 펴진 상태) 자동 송출 (`sim_bridge_node_v6.py`)**:
   - 브릿지 노드가 초기화되자마자 비전 영상 수신 여부와 관계없이 0번 틱(Tick 0)부터 100Hz 주기로 기본 영점 각도(손가락이 곧게 펴진 상태)를 지속 송출합니다.
   - 로봇 연결 및 스크립트 기동 즉시 손가락이 처지거나 말려있지 않고 **곧게 쫙 펴진 완벽한 영점(Zero Position) 자세로 즉각 켜집니다.**
2. **Abduction(1번째 관절)과 MCP Flexion(2번째 관절)의 완벽한 상호 간섭 제거**:
   - 기존의 3D 벡터 사이각(`angle_between`) 방식은 손가락을 좌우로 벌릴 때(외전) 2번째 관절이 앞뒤로 꺾이는 기구학적 크로스토크가 발생했습니다.
   - 손바닥 수직 법선 벡터 투영(`proj_norm = dot(v_prox, palm_normal)`)을 적용하여, **손가락을 좌우로 넓게 벌려도 2번째 관절은 완벽히 0.0 rad를 유지(앞뒤 꺾임 완전 박멸)**하고 오직 1번째 관절만 부드럽게 좌우로 회전합니다.
3. **단일창 3D Cockpit 위젯 및 카메라 자동 감지 (`teleop_cockpit_v6.py`)**:
...
>
> **1. 🖥️ [신규] 통합 3D 콕핏 UI (단일 윈도우 - 창 1개로 완결, RViz-grade CAD STL 뷰어 탑재)**
> - 카메라 영상 옆 중앙에 **RViz2와 100% 동일한 21개 실물 CAD STL 메쉬를 렌더링하는 고속 OpenGL 3D 뷰어**가 내장되어 있어 **별도의 무거운 RViz2 창 없이 단 1개의 창으로 완결**됩니다.
> ```
> ┌─────────────────────────┬─────────────────────────┬─────────────────────────┐
> │ [좌측] 🎥 카메라 영상     │ [중앙] 🤖 3D CAD 로봇 핸드│ [우측] 🎮 계측 및 제어    │
> │ • 웹캠/리얼센스 RGB 영상    │ • 공식 CAD STL 3D 메쉬  │ • 20-DOF 실시간 각도 바 │
> │ • MediaPipe 21개 스켈레톤  │ • 마우스 좌클릭 360° 회전 │ • 왼손/오른손 [H] 전환  │
> │ • 핀치 거리 실시간 측정   │ • 마우스 우클릭 팬(이동)  │ • Clutch [Space] 고정   │
> │ • 트래킹 상태 인디케이터 │ • 마우스 휠 줌 인/아웃   │ • Record [R] / Tag [S]  │
> │                         │ • 금속성 음영 조명 & 접촉 발광│ • 에피소드 저장 카운터  │
> └─────────────────────────┴─────────────────────────┴─────────────────────────┘
> ```
> ```bash
...
> ./run_cockpit.sh real
>
> # 시뮬레이션 테스트 시 (로봇 없이 가상 3D CAD 손 확인)
> ./run_cockpit.sh sim
> ```
...
### ⚡ 1초 하드웨어 통신 & 관절 엔코더 전수 진단 도구 (`check_hand.sh`)
> **"로봇 손 통신이 잘 되는지, 왼손/오른손이 맞는지, 모터 엔코더가 정상인지 확인하고 싶을 때"**
>
> ROS 2 노드나 무거운 패키지를 올리지 않고도, **단 1초 만에 순수 소켓 통신으로 Modbus TCP 포트 502 연결, 펌웨어 버전, 왼손/오른손 자동 판별, 20-DOF 관절 엔코더 실시간 각도**를 즉시 전수 검사합니다.
...
```text
================================================================
  🦾 Allegro Hand V6 Hardware Communication Diagnostic
  Target: 192.168.1.100:502 (Modbus TCP)
================================================================
[✓] Network Ping: 192.168.1.100 REACHABLE (avg RTT: 0.361 ms)
[*] Connecting to Modbus TCP socket at 192.168.1.100:502...
[✓] TCP Connection: Connected to 192.168.1.100:502 successfully!
[✓] Robot Identity:
...
[✓] Real-time 20-DOF Joint Encoders (Degrees & Radians):
    ...
    Index  (joint10~13) :  +19.2°,   -5.9°,  +11.7°,  +63.0°  | rad: [0.334, -0.103, 0.204, 1.100]
    Middle (joint20~23) :   +8.1°,   -5.5°,   +6.1°,  +61.1°  | rad: [0.141, -0.097, 0.106, 1.066]
    Ring   (joint30~33) :   +9.2°,   -4.0°,  +20.6°,  +66.4°  | rad: [0.161, -0.069, 0.359, 1.158]
    Pinky  (joint40~43) :   +4.0°,  -10.0°,  +21.6°,  +55.1°  | rad: [0.071, -0.175, 0.377, 0.962]

================================================================
  🎉 RESULT: Allegro Hand V6 Hardware Communication is 100% HEALTHY!
================================================================
```
...
- **심볼릭 링크 런처 (홈 디렉토리 `~/`)**:
  - `~/check_hand.sh` : ⚡ 1초 하드웨어 통신 & 관절 엔코더 전수 진단 도구
  - `~/run_cockpit.sh` : 🖥️ 통합 3D 콕핏 (단일 창, CAD STL 3D 뷰어 내장)
  - `~/run_dashboard.sh` : 📊 클래식 2D 대시보드 + RViz2 분리 창 뷰어
  - `~/run_v6.sh` : 🚀 V6 올인원 텔레옵 런처
  - `~/run_v6_gui.sh` : 🎛️ 원익 공식 하드웨어 진단 GUI
