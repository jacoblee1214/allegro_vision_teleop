...
|---|---|---|---|
| **`joint00`** | Base Opposition (대립/외전) | 대립 시 **양수(`+0.05 ~ +1.40 rad`)** | 대립 시 **양수(`+0.05 ~ +1.40 rad`)**<br/>*(검지 방향으로 손바닥 전면 스윕)* |
| **`joint01`** | Elevation / Inward Swing (거상/스윙) | 손바닥 쪽 스윙 시 **음수(`-0.10 ~ -0.75 rad`)** | 손바닥 쪽 스윙 시 **음수(`-0.10 ~ -0.75 rad`)**<br/>*(손바닥 안쪽으로 전방 거상)* |
...
> **양손 대칭 키네마틱스 및 기저 회전축(`joint00`/`joint01`) 정렬 완료**:
> 1. **기저 회전축 정렬**:
>    - 원익 공식 URDF는 왼손의 축을 이미 대칭화해 놓았으므로, **양손 모두 `q00`은 양수(`+`), `q01`은 음수(`-`)**로 인가될 때 검지 끝과 정확히 만나 자연스러운 핀치(Pinch) 및 파워 그립을 형성합니다.
>    - `joint00`을 음수로 반전할 경우 엄지가 바깥쪽으로 빠져 다른 네 손가락과 나란히 아래를 향하게 되므로, 순정 대칭 양수 각도로 완벽히 복원하였습니다.
> 2. **마디 굽힘(`joint02`, `joint03`) 과신전 방지**:
>    - `q02`, `q03`은 양수(`+`)일 때 앞으로 굽혀지며, 음수 인가 시 발생하던 역방향 꺾임(Hyperextension)이 완전히 해소되었습니다.

### 3. 🛡️ URDF 버전 분리 관리 전략 (하드웨어 벤더 공식 업데이트 대비)
- **배경**: 원익 로보틱스(제조사)에서 차후 펌웨어나 패키지 공식 업그레이드를 제공할 때 `allegro_hand_v6_left.urdf` 파일명을 그대로 덮어쓰게 됩니다.
- **버전 분리 전략**:
  - 원익 공식 파일(`src/allegro_hand_v6/.../allegro_hand_v6_left.urdf`)은 순정 원본 상태를 유지합니다.
  - 본 텔레옵 시스템 검증 버전은 **`allegro_vision_teleop/urdf/allegro_hand_v6_left_v6.1_teleop.urdf`**로 명확한 버전을 붙여 레포지토리 내에서 자체 관리합니다.
  - 대시보드([`teleop_dashboard_v6.py`](file:///home/jake/humble_ws/allegro_vision_teleop/teleop_dashboard_v6.py))는 `v6.1_teleop.urdf`를 최우선 로드하므로, **향후 제조사 공식 업데이트 패키지를 덮어쓰더라도 텔레옵 시스템의 기구학 설정이 절대 깨지지 않습니다.**
