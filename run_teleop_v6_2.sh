#!/usr/bin/env bash
# ==============================================================================
# run_teleop_v6_2.sh — All-in-One Launcher for Allegro Hand V6 Vision Teleoperation (v6_2)
#
...
if [ ! -f "/.dockerenv" ] && [ "${RUN_ON_HOST:-0}" != "1" ]; then
    ...
    exec docker exec $DOCKER_FLAGS \
        -e DISPLAY="${DISPLAY:-:1}" \
        ros_humble_dev /home/humble_ws/allegro_vision_teleop/run_teleop_v6_2.sh "$@"
fi
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --no-rviz)
            ...
            ;;
        -h|--help)
            echo "Usage: ./run_v6_2.sh [nodes|sim|real] [UI options] [HW options]"
            echo ""
            echo "Modes:"
        ...
        *)
            ...
            echo "Run './run_v6_2.sh --help' for usage."
            exit 1
            ;;
    esac
done
...
pkill -f 'teleop_cockpit_v6(_[12])?\.py|teleop_dashboard_v6(_1)?\.py|vision_tracker_v6\.py' 2>/dev/null || true
sleep 0.2
...
" 2>/dev/null || true)
        ...
        if [[ "$DESCRIPTOR" =~ modbus_tcp:([0-9.]+):([0-9]+) ]]; then
            ...
            if ! ping -c 1 -W 1 "$TARGET_IP" >/dev/null 2>&1; then
                ...
                echo "[!] (Tip: If testing without physical hand, run 'run_v6_2 sim' instead)"
                echo "--------------------------------------------------------"
            else
...
case "$UI_MODE" in
    cockpit)
        ...
        $COCKPIT_GL_ENV python3 "$SCRIPT_DIR/teleop_cockpit_v6_2.py" --device "$DEVICE" --fps "$FPS" --hand "$HAND_SIDE" --mode "$MODE" &
        PIDS+=($!)
        ;;
    classic)
