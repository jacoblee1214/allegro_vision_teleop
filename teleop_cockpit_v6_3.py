...
if __name__ == "__main__" and not os.path.exists("/.dockerenv") and os.environ.get("RUN_ON_HOST", "0") != "1":
    ...
    try:
        ...
        if container_name in res.stdout.split():
            ...
            script_path = "/home/humble_ws/allegro_vision_teleop/teleop_cockpit_v6_3.py"
...
TOPIC_JOINT_STATES = "/allegro/joint_states_urdf"  # v6_1: URDF coordinates, republished by sim_bridge_node_v6_1
...
class RobotHand3DWidget(QOpenGLWidget):
    ...
    def __init__(self, hand_side: str = "right", parent=None):
        ...
        # v6_3: identical to the RViz default view (allegro_hand_v6.rviz Orbit: Yaw 3.9, Pitch 0.2, Distance 0.5,
        # Focal (0, -0.05, 0.1)) for both hands, so the cockpit shows exactly what RViz shows.
        # Orbit camera = focal + d*(cos(p)cos(yaw), cos(p)sin(yaw), sin(p))  ->  azimuth = yaw + 90 deg, elevation = pitch.
        self.azimuth_default = float(np.degrees(3.9) + 90.0 - 360.0)  # -46.5 deg
        self.elevation_default = float(np.degrees(0.2))  # 11.5 deg
        self.distance_default = 0.5
        self.pan_default = [0.0, -0.05, 0.1]
    ...
    def _restore_gl_state(self):
        """
        v6_3: QPainter (HUD overlay at the end of paintGL) leaves the GL state changed after every frame
        (depth test / lighting disabled, its shader program bound, projection reset). Without restoring it,
        every frame after the first rendered with no depth test: inner palm-side parts were drawn over the
        dorsal shell, so the left hand seen from the back looked like a right hand seen from the palm.
        """
        gl.glUseProgram(0)
        ...
        if self._proj is not None:
            ...
            gl.glLoadMatrixf(self._proj)
        gl.glMatrixMode(gl.GL_MODELVIEW)
        gl.glLoadIdentity()
        # Lights in eye space (same as initializeGL, where modelview was identity)
        gl.glLightfv(gl.GL_LIGHT0, gl.GL_POSITION, [0.6, -0.6, 1.2, 0.0])
        gl.glLightfv(gl.GL_LIGHT1, gl.GL_POSITION, [-0.6, 0.6, -0.2, 0.0])

    def resizeGL(self, w: int, h: int):
        self._viewport = (0, 0, w, h)
...
class TeleopDashboardWindow(QMainWindow):
    def __init__(self, device_id: int | str = "auto", target_fps: int = 60, hand_side: str = "right", mode: str = "real") -> None:
        ...
        # Spin ROS in a background thread so /joint_states (100 Hz) is drained continuously.
        # spin_once() inside the camera loop handled only one callback per frame, so the 3D view lagged behind.
    ...
    def init_ui(self) -> None:
        ...
        if self.mode == "real":
            # Real hardware: the physical hand is fixed (auto-detected from Modbus 0x0071)
            self.btn_toggle_hand.setEnabled(False)
            self.btn_toggle_hand.setToolTip("real 모드: 하드웨어가 감지한 손으로 고정 (H 비활성)")
    ...
    def toggle_hand(self) -> None:
        ...
        if self.mode == "real":
            # Switching the model would send the other hand's kinematics (and drop the left MCP offset) to the physical hand
            self.ros_node.get_logger().warn("[H] Hand switch is disabled in real mode (physical hand is fixed).")
            return
