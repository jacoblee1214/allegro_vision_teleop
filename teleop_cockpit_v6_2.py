...
if __name__ == "__main__" and not os.path.exists("/.dockerenv") and os.environ.get("RUN_ON_HOST", "0") != "1":
    ...
    try:
        ...
        if container_name in res.stdout.split():
            ...
            script_path = "/home/humble_ws/allegro_vision_teleop/teleop_cockpit_v6_2.py"
...
class RobotHand3DWidget(QOpenGLWidget):
    ...
    def __init__(self, hand_side: str = "right", parent=None):
        ...
        # v6_2: left hand mirrors the right-hand view (-28 deg) across the palm plane so both are seen from the palm side.
        # (v6_1 used +28 deg for left, which showed the back of the left hand and looked like a right hand.)
        self.azimuth_default = -152.0 if self.hand_side == "left" else -28.0
    ...
    def reset_camera(self):
        self.azimuth = -152.0 if self.hand_side == "left" else -28.0
        self.elevation = self.elevation_default
        self.distance = self.distance_default
        self.pan = list(self.pan_default)
        self.update()
